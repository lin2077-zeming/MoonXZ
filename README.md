...
BCJ 与 `.lzma` 的测试向量全部由 Python 3.14 的 `liblzma`（`xz-utils` 使用的
同一份 C 实现）现场生成，用于交叉验证 MoonXZ 的解码结果。`scripts/interop.py`
同时验证反方向：Python `lzma` 能正确解压 MoonXZ 写出的数据。
...
类型系统、错误处理和内存模型适配。各 filter 的输出以 Python `liblzma` 生成的
向量做交叉验证。参考项目的 BSD-3-Clause 许可证和归属信息见
