...
当前 `LZMA2_COMPRESSED_CHUNK` 是 64 KiB，这是保守值。提高它能把 1 MiB 重复数据的
输出从 1752 字节降到 428 字节（`xz` 为 328 字节），值得做，但本轮尝试没有成功，
下面是把已经确认的事实记下来，避免下次重新踩一遍。

**字段事实（已用计算与实测确认）**

- 压缩块的未压缩大小 = 16 位字段 + 控制字节低 5 位，上限 2^21 - 1 = 2097151。
  这 5 位与 reset 标志不重叠，所以整个范围可用
- 压缩块的**压缩后**大小只有 16 位，上限 65536
- 存储块的未压缩大小也只有 16 位，上限 65536

因此"又大又压不动"的块无法表示，块大小必须自适应：从大值开始，压不下就缩小。

**本轮观察到的现象**

把上限提到 2^21 - 1 并让块大小自减后，编码器写出的流是**合法的**——Python
`liblzma` 能正确解开全部用例（131072 / 262144 / 1048576 字节重复数据），但
MoonXZ 自己的解码器报 `Corrupt("LZMA2 chunk declared N uncompressed bytes but
produced M")`，其中 M 比 N 大 29、270、78 字节不等。也就是说问题在解码器侧，
不在编码器侧，而这个解码器缺陷只在单块未压缩大小超过 64 KiB 时才暴露。

**下次的入手点**

先给解码器补一个"单块解码输出不得超过该块声明大小"的断言并在超限处打印
`segment_start`、目标大小与实际产出，定位是匹配长度越界还是段起点计算错误；
修好后再启用大块。不要在解码器修好之前提高上限——那会产出自己读不了的流。
